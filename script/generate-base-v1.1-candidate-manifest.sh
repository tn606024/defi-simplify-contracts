...
readonly config_path="config/base-v1.1-deployment-candidate.json"
...
require_command() {
  ...
  command -v "$command_name" >/dev/null || {
    echo "$command_name is required to generate the Base v1.1.0 candidate manifest" >&2
    exit 1
  }
}

for required_command in forge cast jq git; do
  require_command "$required_command"
done

[[ -f "$config_path" ]] || {
  echo "Missing deployment candidate config at $config_path" >&2
  exit 1
}

./script/check-foundry-version.sh >/dev/null
./script/check-account-abstraction-revision.sh >/dev/null
./script/check-forge-std-revision.sh >/dev/null
...
if [[ "$configured_foundry_version" != "$pinned_foundry_version" ]]; then
  echo "Deployment candidate Foundry version does not match .foundry-version" >&2
  exit 1
fi
...
if [[ "$expected_build_settings" != "$actual_build_settings" ]]; then
  echo "Deployment candidate does not match the active pinned Foundry build settings" >&2
  exit 1
fi

readonly dependency_lock="config/account-abstraction-v0.9.0.json"
readonly foundry_lock="foundry.lock"
if [[ "$(jq -r '.dependencies.accountAbstraction.commit' "$config_path")" \
  != "$(jq -r '.accountAbstraction.commit' "$dependency_lock")" ]] \
  || [[ "$(jq -r '.dependencies.openzeppelinContracts.commit' "$config_path")" \
  != "$(jq -r '.openzeppelinContracts.commit' "$dependency_lock")" ]] \
  || [[ "$(jq -r '.dependencies.forgeStd.commit' "$config_path")" \
  != "$(jq -r '."lib/forge-std".tag.rev' "$foundry_lock")" ]] \
  ...
  echo "Deployment candidate does not match the dependency and Base EntryPoint locks" >&2
  exit 1
fi

readonly artifact_source_commit="$(jq -er '.artifactSource.commit' "$config_path")"
readonly artifact_source_tree="$(jq -er '.artifactSource.tree' "$config_path")"
if [[ "$(git rev-parse "${artifact_source_commit}^{tree}")" != "$artifact_source_tree" ]]; then
  echo "Deployment candidate artifact source commit does not resolve to the frozen tree" >&2
  exit 1
fi

readonly production_inputs=(
  ".foundry-version"
  "foundry.toml"
  "foundry.lock"
  "remappings.txt"
...
if ! git diff --quiet "$artifact_source_commit" -- "${production_inputs[@]}"; then
  echo "Current production build inputs differ from the frozen artifact source commit" >&2
  exit 1
fi
...
artifact_manifest() {
  ...
  local creation_code_hash runtime_code_hash initcode_hash abi_json_hash
  ...
  abi_json_hash="$(hash_json_file "$abi_path")"
  ...
  jq -n \
    ...
    --arg abiJsonKeccak256 "$abi_json_hash" \
    ...
    '{
      ...
      abiPath: $abiPath,
      abiJsonKeccak256: $abiJsonKeccak256,
      constructor: {
...
jq -nS \
  ...
  --arg releaseVersion "$(jq -er '.releaseVersion' "$config_path")" \
  ...
  --argjson artifactSource "$(jq -ec '.artifactSource' "$config_path")" \
  ...
  --argjson dependencies "$(jq -ec '.dependencies' "$config_path")" \
  --argjson build "$(jq -ec '.build' "$config_path")" \
  --argjson scope "$(jq -ec '.scope' "$config_path")" \
  --argjson security "$(
    jq -ec '.security + {
      notice: "Experimental and unaudited by an independent third party; repository-authored tests, analysis, fork proofs, deployment reproduction, and review evidence are not an independent audit or security guarantee. Total and irreversible loss is possible. Use at your own risk; no warranty is provided."
    }' "$config_path"
  )" \
...
echo "Generated Base v1.1.0 deployment candidate manifest at $output_path"
