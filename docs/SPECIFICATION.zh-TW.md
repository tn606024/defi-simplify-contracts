...
- architecture decisions：ADR-001 記錄 pinned upstream dependency，ADR-002
  記錄 function-local account checkpoint。
...
- exact patched approval 可能未被完整 pull 時，SDK 應提供明確選項，在 flow 尾端
  加入 `approve(spender, 0)` 清理 allowance。adapter 必須保留 zero-first token 的
  要求。這是 SDK policy，不是 account 對「絕無 residual allowance」的保證。
...
immutable。account checkpoint record 必須限於當次 `executeBatchDynamic`
invocation；external target frame 與後續 invocation 都不得觀察到它們。canonical
implementation 以 function-local memory 保存這些 records。execution lock 因為
必須讓 reentrant frame 看見，所以必須使用具 domain separation 的 transient
storage key。
...
- checkpoint records 必須保持在當次 function invocation 內，invocation return 或
  revert 後就不再存在。
- 同一 transaction 的後續 invocation 不得觀察到前一次 invocation 的 checkpoint
  state，即使重用相同 ID 也一樣。

canonical implementation 先加總所有 `checkpointsBefore.length`，配置一個固定
capacity 的 memory array，再只掃描已填入的 prefix 做 duplicate 與 lookup 檢查。
已填入長度就是 presence marker，因此 zero balance 不需要 sentinel，也不需要
invocation counter 或 cleanup list。
...
implementation 可以在同一 call 中依 token 只讀一次 current balance，並在 patch
resolution 與 checkpoint creation 間共用，因為兩個 phase 之間沒有 external
call。若第一個觸發失敗或 short balance read 的 logical consumer 是 patch，必須
revert `PatchBalanceReadFailed(callIndex, patchIndex, token, reason)`；其中
`patchIndex` 是第一個觸發該讀取的 patch index。若第一個 consumer 是 checkpoint，
則使用 `CheckpointBalanceReadFailed(callIndex, checkpointIndex, token, reason)`，
同樣回報第一個觸發 index。cache 不得跨 target call。
...
也使用 wrapper，讓 SDK decode 一致。v1 不會靜默截斷 revert data。惡意 target
可以回傳大到讓 caller 在 copy 時耗盡 gas 的 data，此時 transaction 可能在 encode
`DynamicCallFailed` 前 out of gas。這個接受的 gas-griefing 限制不影響 atomic
rollback，但代表 gas exhaustion 下不保證能取得 indexed attribution。
...
- sequential same-transaction invocations 證明 checkpoint record 是
  function-local，並隔離相同 ID 的重用；
...
- malformed 與 reverting ERC20 `balanceOf` tests，包含完整 call/checkpoint 或
  call/patch index，及 same-call balance read cache 的 first-trigger attribution；
- self-target 與 dynamic reentrancy tests，包含 configured mock EntryPoint reenter
  account 並確實碰到 transient lock；
- 真實 pinned EntryPoint target tests：`depositTo` 仍可使用，而 nested `handleOps`
  在回到 account 前，先被 EntryPoint 自己的 `Reentrancy()` guard 拒絕；
- bounded large-revert-data preservation 與 out-of-gas characterization tests；
